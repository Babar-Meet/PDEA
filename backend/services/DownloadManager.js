...
class DownloadManager {
  ...
  pauseDownload(downloadId) {
    ...
    // Save to pause service
    const pauseService = require('./pauseService');
    pauseService.savePausedDownload(pauseInfo);
    
    // If process is running, terminate it but don't clean temp files
    if (processData && processData.process) {
      const { process } = processData;
      const pid = process.pid;
      
      // Mark as paused so close handler knows not to complete the download
      processData.paused = true;
      
      // Remove from processes map first
      this.processes.delete(downloadId);
  ...
  resumeDownload(downloadId) {
    ...
    // Get paused info
    const pausedInfo = pauseService.getPausedDownload(download.url, download.saveDir);
    if (!pausedInfo) return false;
    
    // Remove from paused list
    pauseService.removePausedDownload(download.url, download.saveDir);
